...
void Base_ImageContrler::FreshCameraParametersToImageCaptureBoard()
{
	...
	Fg_setParameter(m_apc_data.fg, FG_WIDTH, &width_, m_apc_data.dmaIndex);
	Fg_setParameter(m_apc_data.fg, FG_HEIGHT, &heigh_, m_apc_data.dmaIndex);
	Fg_setParameter(m_apc_data.fg, FG_EXPOSURE, &expo_, m_apc_data.dmaIndex);
	Fg_setParameter(m_apc_data.fg, FG_FRAMESPERSEC, &fps_, m_apc_data.dmaIndex);
	Fg_setParameter(m_apc_data.fg, FG_CAMERA_LINK_CAMTYP, &camera_link_type_, m_apc_data.dmaIndex);
}
